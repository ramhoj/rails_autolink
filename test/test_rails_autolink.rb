...
class TestRailsAutolink < Minitest::Test
  ...
  def test_auto_link_email_addres_with_especial_chars
    ...
    email_sanitized = if Rails.version =~ /^3/
      ...
      "re$la*+r-a.o&#x27;rea=l~ly@tenderlovemaking.com"
    else
      "re$la*+r-a.o&#39;rea=l~ly@tenderlovemaking.com"
    end

    # and&amp; not being included in the link is an effect of changed behavour in
    # ActionView#sanitize and might be something we'd like to work around.
    email_result = %{and&amp;<a href="mailto:re%24la%2A%2Br-a.o%27rea%3Dl~ly@tenderlovemaking.com">#{email_sanitized}</a>}
